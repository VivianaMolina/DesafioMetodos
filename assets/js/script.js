...
//Traumatologia - Imprime la tabla de Traumatología
...
//Radiologia - Imprime la tabla de Radiología
...
//Imprime la lista de consultas médicas de Dental separando por un guión cada desplegado
document.write(`<h3>3. Lista de consultas médicas de Dental separando por un guión</h3>`);
// Con map se crea un nuevo array con los atributos de cada objeto concatenados
const listaMediacaDental = dental.map(dental => `${dental.hora} - ${dental.especialista} - ${dental.paciente} - ${dental.rut} - ${dental.prevision}`);

// Con document.write se muestra cada elemento en un párrafo
listaMediacaDental.forEach((elemento) => {
    document.write(`<p>${elemento}</p>`);
});

//Imprimir un listado total de todos los pacientes que se atendieron en el centro médico. 
//Concatenamos radiologia, traumatologia y dental
var listadoPacientes = radiologia.concat(traumatologia, dental);
console.log(listadoPacientes);

//Recorremos la lista concatenada y seleccionamos el nombre del paciente
